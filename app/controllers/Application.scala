...
class Application @Inject()(val userRepo: UserRepository,
  ...
  def signin(emailValue: String = "") = Action.async { implicit request =>
    request.session.get("logged-user") match {
      case Some(userEmail) => {
        Future(Redirect(routes.MainController.index()))
      }
      case None => {
  ...
  def doLogout() = SecureRequest.async {
    implicit request =>
      Future {
        ...
        form => {
          ...
          userRepo.getUser(form.email) flatMap {
            case Some(user) => for {
              ...
              email <- Future {
                ForgotPasswordEmail(host = request.host, url = s"admin/forgot/$url", to = user.email)
              }
  ...
  def changePasswordPage(jwt: String) = Action.async {
    implicit request =>
      val secret = configuration.getString("play.crypto.secret").getOrElse("XXX")
  ...
  def reactivatePage(jwt: String) = Action.async {
    implicit request =>
      val secret = configuration.getString("play.crypto.secret").getOrElse("XXX")
  ...
  def saveNewPassword = Action.async {
    implicit request =>
  ...
  def processUpdate(form: NewSimplePasswordForm): Future[Result] = {
    userRepo.updatePassword(form.email, UserPasswordManager.encryptPassword(form.newPassword)).map {
      x =>
        Redirect(routes.Application.signin())
  ...
  def processReactivation(form: ReactivateForm): Future[Result] = {
    userRepo.setUserActive(form.email).map {
      x =>
        Redirect(routes.Application.signin())
